...
import { Upload, Plus, Trash2, Edit, HardDrive, Cpu, MemoryStick, HardDrive as StorageIcon, Power, Monitor, Wrench, CaseSensitive, Download, FileUp, Save, ListChecks, ShoppingCart, CheckCircle2, AlertTriangle } from "lucide-react";
 ...
 * Styling: Tailwind + shadcn/ui; Animations: Framer Motion
 */
      ...
      <ScrollArea className="max-h-[360px]">
        {items.map((p) => (
          <div key={p.id} className="grid grid-cols-12 items-center px-4 py-2 border-t hover:bg-muted/30 text-sm">
            ...
            <div className="col-span-2">{p.attributes.socket || p.attributes.type || p.attributes.formFactor || "-"}</div>
                  ...
                  <SelectContent>
                    {availableByCat[cat].map(p => (
                      <SelectItem key={p.id} value={p.id}>
                        {p.name} — {baht(p.price)} {p.stock <= 3 && <span className="text-amber-600">(เหลือ {p.stock})</span>}
                      </SelectItem>
                    ))}
