...
function ProductEditor({ initial, onSave }: { initial?: Partial<Product>, onSave: (p: Product) => void }) {
  ...
  <DialogContent className="sm:max-w-[720px]">
    ...
    <div className="space-y-3">
      <Input autoFocus placeholder="พิมพ์คำค้น เช่น 'Ryzen', '27"', 'Windows'" value={searchText} onChange={(e)=>setSearchText(e.target.value)} />
      <div className="text-xs text-muted-foreground">ผลลัพธ์เรียงตาม: {sortMode==='default'?'ค่าเริ่มต้น': sortMode==='priceAsc'?'ราคาต่ำ→สูง': sortMode==='priceDesc'?'ราคาสูง→ต่ำ': sortMode==='nameAsc'?'ชื่อ A→Z':'สต็อกมาก→น้อย'}</div>
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-5 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
        <div className="text-sm text-slate-600">กด <kbd className="px-2 py-1 rounded bg-slate-200">Ctrl/⌘</kbd> + <kbd className="px-2 py-1 rounded bg-slate-200">K</kbd> เพื่อค้นหาอย่างรวดเร็ว</div>
        <div className="flex items-center gap-2">
