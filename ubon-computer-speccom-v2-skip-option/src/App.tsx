...
import { Plus, Trash2, Edit, HardDrive, Cpu, MemoryStick, HardDrive as StorageIcon, Power, Monitor, Wrench, CaseSensitive, Download, FileUp, Save, ListChecks, ShoppingCart, CheckCircle2, AlertTriangle, Sparkles } from "lucide-react";
import * as XLSX from "xlsx";

/**
 * Ubon Computer SpecCom V.2 — single-file React app
...
function ProductEditor({ initial, onSave }: { initial?: Partial<Product>, onSave: (p: Product) => void }) {
  ...
  <DialogContent className="sm:max-w-[720px]">
    ...
    <div className="space-y-3">
      <Input
        autoFocus
        placeholder="พิมพ์คำค้น เช่น 'Ryzen', '27\"', 'Windows'"', 'Windows'"
        value={searchText}
...
// ===== Base Picker =====
function BasePicker({ inventory, selection, onSelect, sortMode }:{ inventory: Product[]; selection: Partial<Record<BaseCategory, Product>>; onSelect:(cat:BaseCategory, product:Product|null)=>void; sortMode: SortMode; }){
  const byCat = useMemo(()=>{
          ...
          {BASE_CATEGORIES.map((cat)=>(
            <div key={cat} className="grid grid-cols-12 items-center gap-2 p-2 rounded-xl hover:bg-muted/40">
              ...
              <div className="col-span-8 flex gap-2">
                <Select value={selection[cat]?.id || "__none__"} onValueChange={(id)=>{ if(id==="__none__"){ onSelect(cat, null); return; } const item = byCat[cat].find(p=>p.id===id) || null; onSelect(cat, item); }}>
                ...
                <Button variant="ghost" onClick={()=>onSelect(cat, null)}>ข้าม</Button>
...
// ===== Summary =====
function Summary({ build }:{ build: BuildState }){
  ...
  return (
    <Card className="shadow-sm">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {(showAll ? BASE_CATEGORIES.map(c=>build.base[c]).map((p,i)=>({p, c: BASE_CATEGORIES[i]})) : BASE_CATEGORIES.map(c=>({p: build.base[c], c}))).map(({p,c}) => (
            p ? (
              <div key={(p as Product).id} className="flex justify-between bg-muted/30 rounded-xl px-3 py-2"><div className="font-medium">{c}: <span className="font-normal">{(p as Product).name}</span></div><div>{baht((p as Product).price)}</div></div>
            ) : (
              showAll ? <div key={String(c)} className="flex justify-between bg-muted/10 rounded-xl px-3 py-2 text-muted-foreground"><div className="font-medium">{c}: <span className="font-normal">— ไม่เลือก —</span></div><div>-</div></div> : null
            )
          ))}
          ...
          {!showAll && baseItems.length===0 && build.addons.length===0 && <div className="text-muted-foreground">ยังไม่ได้เลือกชิ้นส่วน</div>}
        </div>
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="rounded-2xl p-5 mb-3 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 text-white shadow">
        <motion.h1 initial={{opacity:0, y:-8}} animate={{opacity:1, y:0}} className="text-2xl md:text-3xl font-bold tracking-tight">
          Ubon Computer SpecCom V.2 — จัดสเปค/จัดการสินค้า (Excel ได้)
        </motion.h1>
      ...
      <Tabs defaultValue="builder" className="space-y-4">
        ...
        <TabsContent value="builder" className="space-y-4">
          <BasePicker inventory={inventory} selection={build.base} onSelect={selectBase} sortMode={sortMode} />
